...
create fake data following some parameters
...
# your code here
my_simulated_data <- rnorm(500,100,15)
my_simulated_data <- as.data.frame(my_simulated_data)
```

## Ex. 0.c) Talk with your group and note in 1-2 sentences: What do we know of the data drawn from the rnorm()? 
normally distributed data with mean of 100 and stadard deviation fo 15
...
```{r}
ggplot(my_simulated_data, aes(x = my_simulated_data)) +
  geom_histogram(binwidth = 5) +
  labs(
    x = "IQ-Score",
    y = "Count",
    title = "IQ distribution",
    subtitle = "simulated from 500 IQ-scores")
...
```{r}
sd(my_simulated_data$my_simulated_data)
mean(my_simulated_data$my_simulated_data)
```
NOt enough data
...
slope <- 10
x_values <- seq(0,4)
y_value <- intercept + slope*x_values
error <- rnorm(5,0,3.9)
y <- y_value + error

hypdata <- data.frame(y,x_values)

# Make scatterplot
plot(hypdata$x_values, hypdata$y, xlab="stuff",
ylab="what can be predicted by stuff")

# Estimate regression y = a + bx + error
RegData <- stan_glm(y ~ x_values, data=hypdata)

# Add a fitted line to the graph
abline(coef(RegData), col="gray") # needs to be run with the plot() code above - running the whole chunk is the easiest way
...
```{r}
names <- read.csv("../data/Names/data/allnames_clean.csv")
...
girls_data_long <- girls_data %>%
  gather(key = "year", value = "count", -name, -sex, -X) %>%
  mutate(year = as.numeric(sub("X", "", year)))

# Filter for girls
girls_data_long <- filter(girls_data_long, sex == "F")

# Extract the last letter from the names
girls_data_long <- mutate(girls_data_long, last_letter = substr(name, nchar(name), nchar(name)))

# Aggregate data by year and last letter
aggregated_data <- girls_data_long %>%
  group_by(year, last_letter) %>%
  summarise(count = sum(count))

# Create the ggplot graph
ggplot(aggregated_data, aes(x = year, y = count, group = last_letter, color = last_letter)) +
  geom_line() +
  ggtitle("Trend of Girls' Names Based on Last Letter") +
  xlab("Year") +
  ylab("Count") +
  theme_minimal()+
  ylim(0, 900000)  # Adjust the range based on your preference
