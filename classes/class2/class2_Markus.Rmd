...
x_values <- seq(-5,
                5,
                by = 0.1)
#lets calculate som y values by using the function on the x'es
...
hist(n_girls)
```
Next step is to account for twins
```{r}
...
shotfunction <- function(shots,prop){
  ...
  for (i in 1:shots){
    sim_shots[i] <- rbinom(1,1,prop)  } # I am simulatinf one shot at a time
